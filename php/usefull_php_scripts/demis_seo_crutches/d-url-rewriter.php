...
function durOtherReplacer($sContent)
{
	...
	$sContent = str_replace('<strong style="font-size: 22px;">ОСТАВЬТЕ ЗАЯВКУ</strong>', '<span style="font-size: 22px; font-weight: bold;">ОСТАВЬТЕ ЗАЯВКУ</span>',$sContent);

	$sContent = str_replace('<h3><span class="vt_heading1"><span class="vt_heading2">
  Электрика    </span></span></h3>', '',$sContent);

	if($_SERVER['REQUEST_URI']=='/'){
		$sContent = str_replace('<h3><span class="vt_heading1"><span class="vt_heading2"> НАШИ УСЛУГИ</span></span></h3>', '<div style="text-align: center; margin-bottom: 0px; font-size: 48px; "><span class="vt_heading1"><span class="vt_heading2"> НАШИ УСЛУГИ</span></span></div>',$sContent);
       ...
       $sContent = str_replace('<center><h1>Услуги мужа на час!</h1> <br></center>', '<center><div style="font-size: 180%; line-height: 1em; margin: 0px 0; padding: 0px 0px 0px 0px; font-weight: normal;">Услуги мужа на час!</div> <br></center>',$sContent);
      $sContent = str_replace('<h2>При звонке сообщите оператору кодовую фразу "РЕМОНТ НА ГОДА" и получите скидку в 10% на все заказанные услуги.</h2>', '<div style="margin: 0px 0; padding: 0px 0px 0px 0px; font-weight: normal; font-size: 160%; line-height: 1em;">При звонке сообщите оператору кодовую фразу "РЕМОНТ НА ГОДА" и получите скидку в 10% на все заказанные услуги.</div>',$sContent);
      $sContent = str_replace('<h2>Подробности акции уточняйте по телефону.</h2>', '<div style="margin: 0px 0; padding: 0px 0px 0px 0px; font-weight: normal; font-size: 160%; line-height: 1em;">Подробности акции уточняйте по телефону.</div>',$sContent);

	}
	if($_SERVER['REQUEST_URI']=='/elektrik-na-dom.html'){
		$sContent = preg_replace('#<div class="vt_heading_style">(.*)</div>#siU', '',$sContent);
	}
	// $sContent = str_replace('<h1>ВНИМАНИЕ! АКЦИЯ!</h1>', '<h1>Услуги мужа на час </h1>',$sContent);
	// if ($_SERVER['REQUEST_URI'] != '/'){
	// $sContent = str_replace('<h1>Услуги мужа на час </h1>', '',$sContent);
	// }
